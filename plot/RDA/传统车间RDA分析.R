...
bacteria <- read.csv("./data/temp/transcriptome.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
physicochemical <- read.csv("./data/temp/physicochemical.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
mapping <- read.csv("./data/mapping/mapping.csv", header = TRUE, row.names = 1, encoding = "UTF-8", check.names = FALSE)
...
g <- ggplot() + #指定数据、X轴、Y轴，颜色
  geom_point(data = df_rda, aes(x = RDA1, y = RDA2, color = Group), size = 3, shape = 16) +
...
g
# ggsave(filename = "./pdf/RDA traditional all.pdf", g + p, width = 25, height = 20, dpi = 600, units = "cm", device = 'pdf')
